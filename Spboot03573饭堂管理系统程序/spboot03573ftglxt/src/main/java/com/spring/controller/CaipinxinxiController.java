...
public class CaipinxinxiController extends BaseController
{
    ...
    public String getWhere()
    {
        ...
        // 以下也是一样的操作，判断是否符合条件，符合则写入sql 语句
            ...
            where += " AND fenlei ='"+Request.get("fenlei")+"' ";
                ...
                if(!Request.get("shifouxinpin").equals("")) {
            where += " AND shifouxinpin LIKE '%"+Request.get("shifouxinpin")+"%' ";
        }
            return where;
    }
    ...
    public String insert()
    {
        ...
        post.setCaipinzhutu(Request.get("caipinzhutu"));

        post.setXijiexiangtu(Request.get("xijiexiangtu"));

        post.setJiage(Request.getDouble("jiage"));

        post.setZhekoujia(Request.getDouble("zhekoujia"));

        post.setKucun(Request.getInt("kucun"));

        post.setShoumaichuangkou(Request.get("shoumaichuangkou"));

        post.setShifouxinpin(Request.get("shifouxinpin"));

        post.setCaipincailiao(Request.get("caipincailiao"));

        post.setCaipinjianjie(util.DownloadRemoteImage.run(Request.get("caipinjianjie")));
    ...
    */
    ...
    public String update()
    {
        ...
        post.setCaipinmingcheng(Request.get("caipinmingcheng"));
                if(!Request.get("fenlei").equals(""))
        post.setFenlei(Request.get("fenlei"));
                if(!Request.get("caipinzhutu").equals(""))
        post.setCaipinzhutu(Request.get("caipinzhutu"));
                if(!Request.get("xijiexiangtu").equals(""))
        post.setXijiexiangtu(Request.get("xijiexiangtu"));
        ...
        post.setKucun(Request.getInt("kucun"));
            if(!Request.get("shoumaichuangkou").equals(""))
        post.setShoumaichuangkou(Request.get("shoumaichuangkou"));
                if(!Request.get("shifouxinpin").equals(""))
        post.setShifouxinpin(Request.get("shifouxinpin"));
                if(!Request.get("caipincailiao").equals(""))
        post.setCaipincailiao(Request.get("caipincailiao"));
                if(!Request.get("caipinjianjie").equals(""))
        post.setCaipinjianjie(util.DownloadRemoteImage.run(Request.get("caipinjianjie")));
